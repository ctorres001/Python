...
# ======================
# CONFIGURACIÓN GLOBAL - POSTGRESQL
# ======================
...
# Mapeo de tipos de datos PostgreSQL
COLUMN_TYPES_PG = {
...
# Mapeo de nombres originales a nombres PostgreSQL (minúsculas)
...
def detectar_separador(linea_cabecera):
    ...
    for sep in separadores:
        if sep in linea_cabecera and linea_cabecera.count(sep) >= 5:
            return sep
    return '\t'
...
def analizar_archivo_txt(archivo_entrada):
    ...
    if idx_cab is not None:
        ...
        print(f"Número de columnas: {len(cabecera_partes)}")
        print(f"Columnas: {', '.join(cabecera_partes[:10])}...")
    else:
...
def limpiar_archivo_txt(archivo_entrada):
    ...
    if idx_cabeceras_vacias:
        print(f"⚠ Se encontraron {len(idx_cabeceras_vacias)} columnas vacías que serán eliminadas")
    ...
    if filas_corregidas > 0:
        print(f"⚠ Se corrigieron {filas_corregidas} filas con problemas de columnas")
    ...
    if seleccionadas:
        ...
        raise ValueError("No se encontró ninguna columna mapeada en el archivo")

    if faltantes:
        print(f"⚠ Columnas faltantes: {', '.join(faltantes[:5])}")
    ...
    if "BD" in nombre_archivo and any(c.isdigit() for c in nombre_archivo):
        ...
        if match:
            ...
            try:
                ...
                return f"bd_potenciales_{fecha.strftime('%Y%m%d')}"
            except:
                pass
...
def limpiar_nombres_columnas_postgresql(df):
    ...
    for col in df.columns:
        ...
        # PostgreSQL: convertir a minúsculas
...
def cargar_dataframe_a_postgresql_optimizado(df, table_name):
    ...
    try:
        ...
        crear_tabla_bd_potenciales(cursor, table_name)
        conn.commit()
...
def procesar_archivo_completo(archivo_txt, generar_csv=True, cargar_postgresql=True, csv_path=None):
    ...
    try:
        ...
        # Paso 2: Limpiar y cargar en DataFrame
        print("\n" + "=" * 60)
        ...
        # Paso 6: Cargar a PostgreSQL (opcional)
